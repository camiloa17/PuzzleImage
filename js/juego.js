...
var instrucciones = ["Utilizar las flechas para mover las piezas hacia el espacio vacio.", "Ordenar las piezas hasta alcanzar la imagen objetivo."];
...
function mostrarInstrucciones(instrucciones) {
  ...
  for (let i = 0; i < instrucciones.length; i++) {
    ...
    let text = document.createTextNode(instrucciones[i]);
    node.appendChild(text);
    document.getElementById("lista-instrucciones").appendChild(node);
  }
...
function updateMovement(direction) {
  movimientos.push(direction);
  actualizarUltimoMovimiento(direction);
}
...
function chequearSiGano() {
  ...
  for (x = 0; x < grilla.length; x++) {
    for (y = 0; y < grilla[x].length; y++) {

      if (grilla[x][y] !== grillaGanadora[x][y]) {
        return false;
      }
...
function mostrarCartelGanador() {
  ...
  //Check if he wants to start again and if he does it execute mezclar Piezas adding the level variable. 
  setTimeout(function () {
    let startAgain = window.confirm("Quieres volver a empezar?");
    if (startAgain === true) {
      level++;
      mezclarPiezas(repetir + level);
      console.log(level);
    }
...
function intercambiarPosicionesGrilla(filaPos1, columnaPos1, filaPos2, columnaPos2) {
  let posicionTemporal = grilla[filaPos1][columnaPos1];
